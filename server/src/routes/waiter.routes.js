...
router.get('/orders/ready', (req, res, next) => waiterController.getReadyOrders(req, res, next));
...
router.post('/orders/:id/accept', (req, res, next) => waiterController.acceptOrder(req, res, next));

// T473: POST /api/waiter/orders/:id/reject - Reject a pending order
router.post('/orders/:id/reject', validate(rejectOrderSchema), (req, res, next) =>
  waiterController.rejectOrder(req, res, next)
);
...
router.get('/tables', (req, res, next) => waiterController.getTableStatus(req, res, next));

// Bill Management Routes

// T475: POST /api/waiter/bill/generate - Generate bill for an order
router.post('/bill/generate', validate(generateBillSchema), (req, res, next) =>
  waiterController.generateBill(req, res, next)
);

// GET /api/waiter/bill/:orderId - Get bill for an order
router.get('/bill/:orderId', (req, res, next) => waiterController.getBill(req, res, next));

// T476: POST /api/waiter/bill/pay - Record payment for an order
router.post('/bill/pay', validate(recordPaymentSchema), (req, res, next) =>
  waiterController.recordPayment(req, res, next)
);
